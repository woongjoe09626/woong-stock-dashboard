...
# 🚨 조대표님의 구글 웹 앱 URL을 여기에 꼭 다시 넣어줘!
...
class UpdateItem(BaseModel):
    code: str
    buy_price: float  # 미국 주식 소수점 대비 float 변경
    qty: int
            ...
            function updateDashboard() {
                fetch("/api/market")
                    ...
                    .then(data => {
                        ...
                        document.getElementById("kospi-text").className = "market-card " + (parseFloat(data.kospi.change) > 0 ? "up" : "down");
                        document.getElementById("kosdaq-text").innerText = data.kosdaq.price + " (" + data.kosdaq.change + "%)";
                        document.getElementById("kosdaq-text").className = "market-card " + (parseFloat(data.kosdaq.change) > 0 ? "up" : "down");
                        ...
                        if (myChart) {
                            ...
                            myChart = new Chart(ctx, {
                                type: 'doughnut',
                                data: { labels: labels, datasets: [{ data: chartData, backgroundColor: ['#6366f1', '#a855f7', '#ec4899', '#10b981', '#f59e0b', '#3b82f6'], borderWidth: 0 }] },
